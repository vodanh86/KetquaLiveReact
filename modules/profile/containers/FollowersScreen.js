...
class FollowersScreen extends React.Component {
    ...
    _fetchUser = async () => {
        if(this.state.fetching === false){
            ...
            if(this.state.users.length > 0){
                min_id = await this.state.users[this.state.users.length - 1].follower_id;
            }
            let users = await GetFollowersAPI(this.props.navigation.getParam('id'), min_id, this.state.limit);
    ...
    render() {
        if(this.state.loading === true){
            ...
            return (<View style={styles.container}><Text style={styles.noData}>Chưa có ai theo dõi</Text></View>);
        }else{
            return (<View style={styles.container}>
                <FlatList
                    style={styles.listFollower}
                    data={this.state.users}
                    keyExtractor={(item, index) => item.follower_id}
