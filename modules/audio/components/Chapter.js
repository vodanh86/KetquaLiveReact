...
class Chapter extends Component {
    ...
    getMega = async () => {
        const novelId = this.props.navigation.state.params.novel.id;
        if(this.state.fetching === false){
            ...
            let result = await callAPI('novel/detail?novelId=' + novelId)
            if(result.error === 0){
                    ...
                    <View style={styles.lotoRow}>
                        <View style={styles.lotoNumber}>
                            <View style={styles.lotoNumberSpec}>
                                <Text style={styles.lotoNumberNormal}>{item.name}</Text>
                            </View>
                        </View>
                    </View>
                    </TouchableOpacity>
...
export default connect(mapStateToProps, mapDispatchToProps, null, {forwardRef: true})(Chapter);
